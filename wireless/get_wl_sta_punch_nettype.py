...
def view_punch_by_os(os):

	fig = plt.figure()
	fig.canvas.set_window_title('穿透成功率(无线)-按网络类型分-' + os );
	...
	for k, v in iter(list(isps.items())):
		view_punch({'net': "3G", 'os':os, 'isp' : k}, v, 0)

	for k, v in iter(list(isps.items())):
		view_punch({'net': "wifi", 'os':os, 'isp' : k}, v, 1)
